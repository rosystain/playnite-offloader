...
## 8. 二期候选（2026-09 已拍板/已落地，其余明确不做）

- ✅ **远端完整性校验增强（已落地）**：① 释放确认框**不做前置扫描**（避免 UNC 遍历造成确认前的主动等待，确认框仅列游戏名）；② 终推成功后删本地前**逐文件硬校验**：远端必须包含本地每个文件且大小一致（`LOCoffloaderErrRemoteMismatch` 带文件数+样例路径），不一致则抛错不删；通过后进度行显示「校验通过：N 文件/大小」（`LOCoffloaderProgVerifyPassed`，复用已构建映射，零额外扫描）。**远端多余文件放行**（与「永不删除远端」约定一致，归档天然积累 extras，文件数相等判据已被废弃）。任一侧映射失败时不硬拦（放行 + warn），避免把统计抖动误判为校验失败锁死释放。
- ✅ **恢复目标不存在时的改良交互（此前已落地）**：`ResolveRestoreTarget` 回退快照、原路径失效时弹重选。**孤儿批量清扫明确不做**（单条移除已有，`dbGame==null` 判孤儿有误报风险，批量删除危险）。
- ✅ **恢复前目标盘空间预检（已落地，硬拦截；取不到空间信息时放行）**：`RobocopySyncService.TryGetFreeBytes`（`DriveInfo`，仅支持本地盘/盘符，**UNC 不支持→放行**）；`RestoreToTarget` 拉取前远端总字节 > 目标盘剩余则抛 `LOCoffloaderErrInsufficientSpace` 拒绝恢复。**「释放前磁盘预检」不做**（释放是删本地，不占空间，语义存疑）。
- ❌ **推送失败重试明确不做**：手动重推（立即推送）已覆盖，自动重试可能掩盖真实失败或把半截文件再覆盖一遍，违背「无自动同步」原则（§3.4）。

> 完整性硬校验的核心动机是 §5.7 的 `/XO` 半截文件陷阱：终推中断可能留下时间戳偏新、跳过后续推送的损坏远端文件；本地↔远端文件数/字节比对可精准拦截（非误报）。
